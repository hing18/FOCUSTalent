...
use Illuminate\Support\Facades\DB;
  if($opt==1)
  {
      ...
      $result= DB::table('db_headcontrol.estructuras')
          ->select( 'id','codigo','nameund')
          ->where('id_sup', '=', $sel_grp)
          ->where('status', '=', "true")
          ->orderby('nameund')
          ->get();
          echo($result);
  ...
  if($opt==2)
  {
      $sel_ue=$_REQUEST['sel_ue'];
    // $data_vestruc= DB::table('vestructuraspos')->get();
      ...
      echo '<table class="table table-sm table-hover table-bordered">
          <thead class="bg-primary">
            <tr>
              <th scope="col" class="text-center text-light bg-primary">UNIDAD ECONÓMICA</th>
  ...
  #### ACTUALIZANDO UNIDAD
  if($opt==4)
    { $idund=$_REQUEST['idund'];
      $codigo=$_REQUEST['codigo'];
      $nameund=$_REQUEST['nameund'];
      $id_tipo=$_REQUEST['id_tipo'];
      $id_sup=$_REQUEST['id_sup'];
  ...
  if($opt==6)
  {
    DB::beginTransaction();
      try {
        ...
        $data['nameund'] = $_REQUEST["nameund"];
        $data['id_sup'] = $_REQUEST["id_sup"];
  ...
  {
      $sel_ue=$_REQUEST['sel_ue'];
      $id_unisecc=$_REQUEST['id_unisecc'];
      ...
      echo '<table class="table table-sm table-hover table-bordered">
          <thead class="bg-primary">
            <tr>
              ...
              <th class="text-center text-light bg-primary align-middle"><small>PARTICIPANTES</small></th>
            </tr>
          </thead>
          <tbody><tr>';
                  $unidad = "";
  ...
  if($opt=='sol_vac_listcolab')
  {
    $cod_pue="-";
    $sel_ue=$_REQUEST['sel_ue'];
    if(isset($_REQUEST['cod_pue']))
    { $cod_pue=$_REQUEST['cod_pue'];}
