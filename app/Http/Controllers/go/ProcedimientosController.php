...
class ProcedimientosController extends Controller
{
    public function show(Request $request)
        ...
        if($opt=='sol_vac')
        {
            ...
            [$sel , $tienda] = explode('-', $data['sel_ue']);
            // Obtener la estructura inicial para determinar el tipo
            ...
            if ($estructura) {
                $tipo = $estructura->id_tipo;
                $titulo_col='DEPARTAMENTOS';
                if($tienda==1){$titulo_col='TIENDAS';}
                    // Dependiendo del tipo, buscamos el "grupo superior" según los niveles jerárquicos
                    if ($tipo == 4) { // Si el tipo es 4, buscamos el nivel 2 (e2)
                        $sel_ue = DB::table('estructuras as e4')
                            ->leftJoin('estructuras as e3', 'e3.id', '=', 'e4.id_sup')
                            ->leftJoin('estructuras as e2', 'e2.id', '=', 'e3.id_sup')
                            ->where('e4.id', $sel)
                            ->where('e4.tienda', $tienda)
                            ->where('e4.id_tipo', $tipo)
                            ->value('e2.id'); // Traemos el id del "abuelo"
                            
                    }
                    elseif ($tipo == 3) { // Si el tipo es 3, buscamos el nivel 1 (e3)
                        $sel_ue = DB::table('estructuras as e3')
                            ->leftJoin('estructuras as e2', 'e2.id', '=', 'e3.id_sup')
                            ->where('e3.id', $sel)
                            ->where('e3.id_tipo', $tipo)
                            ->value('e2.id'); // Traemos el id del "padre"
                    }
                    else { // Si el tipo es 2, el nivel más bajo
                        $sel_ue = DB::table('estructuras as e2')
                            ->where('e2.id', $sel)
                            ->where('e2.id_tipo', $tipo)
                            ->value('e2.id'); // Traemos el id directamente
                    } 
            }
            ...
            $result = DB::select('CALL HC (?,?,?,?,?,?)', [$sel_ue, $tienda, $tipo, $sel, Auth::user()->id, Auth::user()->rol]);

            // Ordenar por NOM_SECC ascendente
            $result = collect($result)->sortBy('NOM_SECC')->values()->all();
                    

            echo '<table class="table table-sm table-hover">
                <thead class="bg-primary">
                    <tr>                    
                    <th class="text-center bg-info align-middle text-light"><small>'.$titulo_col.'</small></th>
                    <th class="text-center bg-info align-middle text-light"><small>POSICIONES</small></th>
                    <th class="text-center bg-info align-middle text-light"><small>APRO.</small></th>
                    <th class="text-center bg-info align-middle text-light"><small>REAL</small></th>
                    <th class="text-center bg-info align-middle text-light"><small>VACANTES</small></th>
                    <th class="text-center bg-info align-middle text-light"></th>
                    </tr>
                </thead>
                <tbody><tr>';
                        $unidad = "";
            ...
            //var_dump($result);
                ...
                {     
                    ...
                    // PPSICIONES
                            $check='';
                                    ...
                                    $sum_real=$sum_real + $vestruc->REALL;
                                    
                                    echo' <td class="align-middle"><small><span id="div_nom_puest_'.$vestruc->COD_PUE.'">'.$vestruc->COD_PUE." - ".$vestruc->DESC_PUE.'</span></small></td>
                                            <td class="text-center align-middle bg-light text-secondary"><b>'.$vestruc->APROBADO.'</b></td>
                                            <td class="align-middle bg-light">
                                                <div class="position-relative editlink" style="width: 100%" onclick=modalcolab("'.$cod_uni.'","'.$vestruc->COD_PUE.'") data-bs-toggle="modal" data-bs-target="#modalcolab">
                                                <div class="position-absolute top-50 start-50 translate-middle"><b>'.$vestruc->REALL.'</b></div>
                                                </div>
                                            </td>
                                            ...
                                            <td class="text-center align-middle bg-light"><button type="button" class="btn btn-sm btn-outline-secondary" onclick=modalsolicitud("'.$cod_uni.'","'.$vestruc->COD_PUE.'") data-bs-toggle="modal" data-bs-target="#modalsoli"><i class="fas fa-plus-circle pe-1"></i>Solicitar</button> </td>
                                            </tr>';
                                    }
                                    else {
                ...
                echo '
                    ...
                    <tr >
                    <th class="text-center bg-info align-middle text-light" colspan="2"> TOTAL</th>
                    <th class="text-center bg-info align-middle text-light"<b>'.$sum_apro.'</b></th>
                    <th class="text-center bg-info align-middle text-light"><b>'.$sum_real.'</b></th>
                    <th class="bg-info"></th>
                    <th class="bg-info"></th>
                    </tr>
                </table>'; 
            ...
            { $cod_pue=$data['cod_pue'];}
                    ...
                    $result = DB::table('m_empleados as col')
                    ->select(
                        'col.id as NO_EMPLE',
                        'col.num_doc as CEDULA',
                        'col.prinombre as NOMBRE_SEP',
                        'col.priapellido as APELLIDO_SEP'
                    )
                    ->where('col.id_posicion', $cod_pue)
                    ->orderBy('col.prinombre', 'asc')
                    ->orderBy('col.priapellido', 'asc')
                    ->get();
        ...
        if ($opt == 10) {
            ...
            $result = DB::table('estructuras as g')
            ->select([
                'g.id as COD_GRUPO',
                'g.nameund as NOM_GRUPO',
                'g.codigo as CODIGO_GRP',

                'u.id as COD_UNI',
                'u.codigo as CODIGO_UNI',
                'u.nameund as NOM_UNI',

                'dpto.id as COD_DEPTO_SUC',
                'dpto.codigo as CODIGO_DEPTO_SUC',
                'dpto.nameund as NOM_DEPTO_SUC',

                's.id as COD_SECC',
                's.codigo as CODIGO_SECC',
                's.nameund as NOM_SECC',

                'p.id as COD_PUE',
                'p.codigo as CODIGO_PUE',
                'p.descpue as DESC_PUE',
                'p.iddf as ID_DF',

                DB::raw('COALESCE(cp.cant, 0) as cantidad_p'),

                DB::raw('COALESCE(cu.total, 0) as CONT_UNI'),
                DB::raw('COALESCE(cd.total, 0) as CONT_DEPTO_SUC'),
                DB::raw('COALESCE(cs.total, 0) as CONT_SECC'),
            ])
            ->join('estructuras as u', 'g.id', '=', 'u.id_sup')
            ->leftJoin('estructuras as dpto', 'u.id', '=', 'dpto.id_sup')
            ->leftJoin('estructuras as s', 'dpto.id', '=', 's.id_sup')
            ->leftJoin('posiciones as p', function ($join) {
                $join->on('s.id', '=', 'p.iduni')
                    ->where('p.status', '=', 'true');
            })
            ->leftJoin(DB::raw('(select id_df, count(*) as cant from entrevistas_fun_preguntas group by id_df) as cp'), 'cp.id_df', '=', 'p.iddf')

            # Conteo por unidad
            ->leftJoin(DB::raw('(select u.id, count(p.id) as total
                                from estructuras u
                                join estructuras d on u.id = d.id_sup
                                join estructuras s on d.id = s.id_sup
                                left join posiciones p on s.id = p.iduni and p.status = "true"
                                group by u.id) as cu'), 'cu.id', '=', 'u.id')

            # Conteo por depto
            ->leftJoin(DB::raw('(select d.id, count(p.id) as total
                                from estructuras d
                                join estructuras s on d.id = s.id_sup
                                left join posiciones p on s.id = p.iduni and p.status = "true"
                                group by d.id) as cd'), 'cd.id', '=', 'dpto.id')

            # Conteo por sección
            ->leftJoin(DB::raw('(select s.id, count(p.id) as total
                                from estructuras s
                                left join posiciones p on s.id = p.iduni and p.status = "true"
                                group by s.id) as cs'), 'cs.id', '=', 's.id')
            ...
            ->get();
                    ...
                    echo '<table class="table table-sm table-hover table-bordered">
                        <thead class="bg-primary">
                            <tr>
                                <th class="text-center text-light bg-primary">ÁREA / CADENA</th>
                                <th class="text-center text-light bg-primary">DEPARTAMENTO / SUCURSAL</th>
                                <th class="text-center text-light bg-primary">POSICIONES</th>
                                <th class="text-center text-light bg-primary">PREGUNTAS</th>
                                <th class="text-center text-light bg-primary">EDITAR</th>
                            </tr>
                        </thead>
                        <tbody>';
                    ...
                    foreach ($result as $vestruc) {
                        ...
                        // ÁREA / CADENA -> DEPARTAMENTO / SUCURSAL
                        if ($depto != $vestruc->NOM_DEPTO_SUC) {
                            $depto   = $vestruc->NOM_DEPTO_SUC;
                            $colspan = max(1, $vestruc->CONT_DEPTO_SUC); // nunca 0
                            echo "<td class='align-middle bg-white' rowspan='{$colspan}'>
                                    <small>{$depto}</small>
                                </td>";
                        }

                        // DEPARTAMENTO / SUCURSAL -> SECCIÓN
                        if ($secc != $vestruc->NOM_SECC) {
                            $secc    = $vestruc->NOM_SECC;
                            $colspan = max(1, $vestruc->CONT_SECC ?? 1); // nunca 0
                            echo "<td class='align-middle bg-white' rowspan='{$colspan}'>
                                    <small>" . ($secc ?? '-') . "</small>
                                </td>";
                        }
                        ...
                        // POSICIONES
                        if ($vestruc->COD_PUE) {
                            $cant = $vestruc->cantidad_p > 0 ? $vestruc->cantidad_p : '-';
                            echo "
                                <td class='align-middle bg-white'>
                                    <small>{$vestruc->COD_PUE}- 
                                    <span id='pue_{$vestruc->COD_PUE}'>{$vestruc->DESC_PUE}</span>
                                    </small>
                                </td>
                                <td class='align-middle text-center'>{$cant}</td>
                                <td class='align-middle text-center'>
                                    <button type='button' class='btn btn-sm btn-outline-primary px-1 py-0' 
                                            onclick='edit_preg({$vestruc->ID_DF},{$vestruc->COD_PUE})'>
